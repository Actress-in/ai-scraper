/**
 * ==============================
 * AI Scraper Builder - Google Apps Script (修正版)
 * ==============================
 */
...
// 列番号の定義
const COLUMNS = {
  TARGET_URL: 8,      // H列: ターゲットURL
  STATUS: 18,         // R列: ステータス
  RESULT: 22,         // V列: 結果
  ID: 23              // W列: ID (JobID/ScraperID)
};
...
function executeRow(row) {
  ...
  const targetUrl = sheet.getRange(row, COLUMNS.TARGET_URL).getValue(); // H列: ターゲットURL
  ...
  // ステータスを「処理中」に更新
  sheet.getRange(row, COLUMNS.STATUS).setValue('処理中');
  sheet.getRange(row, COLUMNS.STATUS).setBackground('#FFF3CD');
  ...
  try {
    ...
    if (result.success) {
      ...
      // ジョブIDを記録
      sheet.getRange(row, COLUMNS.ID).setValue(jobId);
    ...
    sheet.getRange(row, COLUMNS.STATUS).setValue('エラー');
    sheet.getRange(row, COLUMNS.STATUS).setBackground('#F8D7DA');
    sheet.getRange(row, COLUMNS.RESULT).setValue(`Error: ${error.message}`);
...
function createResultSheet() {
  ...
  const scraperIdOrJobId = sheet.getRange(row, COLUMNS.ID).getValue();
